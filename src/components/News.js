...
const News = (props) => {
    ...
    document.title = `NewsGo - ${(capitalFirst(props.category))}`;
    ...
    return (
        <>
            <h1 className='text-center' style={{ margin: '70px 0 20px 0' }}>NewsGo - Top Headlines in {(capitalFirst(props.category))}</h1>
            {loading && <Loading />}
            <InfiniteScroll
